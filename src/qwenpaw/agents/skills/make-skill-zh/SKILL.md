...
计划阶段除 `python scripts/create_plan.py` 保存计划记录外只读：依据对话证据和已有产物判断，不执行或探测候选工作流，不创建 package 文件，也不初始化 draft。通过 stdin 传入 workspace 和候选计划：
...
脚本将每份计划独立保存在 `<workspace>/.qwenpaw/make-skill/plans/<plan_id>/plan.json`，返回 `plan_id` 和规范化 `plan`。保留该 ID 用于构建；计划产物存在不代表用户已批准。取消规划时保留计划记录，不创建 draft。
...
- 用户修改后合并最新反馈、增加 `revision`、重跑 `create_plan.py` 并展示完整返回结果，等待批准。使用新返回的 `plan_id`；旧批准不适用于修改后的计划。
- 取消时停止。区分知悉与批准；用户意图不明确时，只追问一次简短确认并等待回复。
...
初始化会读取已保存的计划，不接受内联替代计划。计划缺失或无效时返回规划阶段，不继续构建。

`execution` 决定由当前 agent 还是后台 subagent 完成 Skill 创建。选择 `background` 时，初始化后将完整批准计划、最新纠正、workspace 路径和 draft 路径交给通用 subagent，由其编写文件、校验、执行已批准的行为测试并发布，不再请求批准。完成后报告创建结果；除已批准的行为测试外，运行生成的 Skill 需用户另行要求。
