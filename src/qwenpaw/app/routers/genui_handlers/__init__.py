...
A user action on any generative-UI surface arrives as one A2UI
``ClientAction``. We dispatch by ``surfaceId`` prefix to a registered handler,
so a new
...
should be applied to the surface. Handlers are synchronous (file IO); raise
``ValueError`` for bad input so the router can map it to HTTP 400.
"""
...
def dispatch(workspace: "object", action: ClientAction) -> list[Envelope]:
    ...
    raise ValueError(
        f"no genui action handler for surface {action.surface_id!r}",
    )


# Import side-effect: register the built-in handlers.
# pylint: disable=wrong-import-position
from . import task_plan_handler  # noqa: E402,F401
