...
└── workspaces/
    ...
    │   ├── SOUL.md          # 核心身份与行为原则
    │   ├── skills/          # 当前 workspace 的本地技能
    │   ├── skill.json       # 当前 workspace 的技能状态
    │   └── memory/          # 记忆文件
    └── abc123/              # 其他智能体工作区
        └── ...
├── skill_pool/              # 本地共享技能池
...
| `skills/`            | 当前 workspace 中可用的技能                  |
| `skill.json`         | 当前 workspace 的启用状态、频道和技能元数据  |
| `memory/`            | 记忆文件（自动管理）                         |
...
技能扩展了 Agent 的能力。当前模型中主要分布在两个位置：

| 目录                          | 说明                                                           |
| ----------------------------- | -------------------------------------------------------------- |
| 内置（源码中）                | 随 CoPaw 一起发布——docx、pdf、pptx、xlsx、news、email、cron 等 |
| `~/.copaw/skill_pool/`        | 本地共享池，包含内置和共享技能                                 |
| `~/.copaw/workspaces/{agent_id}/skills/` | 某个 workspace 中实际存在的技能                    |

每个技能是一个目录，里面有 `SKILL.md` 文件（YAML front matter 中需包含 `name` 和
`description`），以及可选的 `references/` 和 `scripts/` 子目录。技能还可以声明
`metadata.requires.bins` 和 `metadata.requires.env`；CoPaw 会将它们透出为
`require_bins` 和 `require_envs` 元数据，而不是作为硬性的启用/禁用门槛。

某个 workspace 中技能是否生效，由
`~/.copaw/workspaces/{agent_id}/skill.json` 控制。
